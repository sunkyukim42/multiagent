...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate a Task 8 research Markdown report.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        markdown = render_research_report(summary, args.report_id)
        if contains_secret(markdown):
            raise ValueError("research report must not contain raw secret values")
        ...
        return 1
    print(f"ResearchReport: {output_path}")
    return 0
